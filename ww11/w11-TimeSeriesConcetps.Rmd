...
More formally, a **strictly stationary stochastic process** is one where given $t_1, \cdots, t_{\mathcal{l}}$ the joint statistical distribution of $X_{t_1} , \cdots ,X_{t_\mathcal{l}}$ is the **same** as the joint statistical distribution of $X_{t_1 + \tau} , . . . ,X_{t_\mathcal{l}+\tau}$ for all $\mathcal{l}$ and $\tau$.

Thus, <font color = "red">**time series with trends, or with seasonality, are not stationary**</font> — the trend and seasonality will affect the value of the time series at different times.
...
The following figure shows the comparison between cyclic and seasonality series.

```{r echo = FALSE, fig.align='center', out.width="50%"}
include_graphics("w11-cyclic-seasonality.jpg")
```
...
```{r echo = FALSE, fig.align='center', fig.width=5, fig.height=5}
include_graphics("w11-naive.jpg")
```
...
```{r echo = FALSE, fig.align='center', fig.width=5, fig.height=5}
include_graphics("w11-NaiveSeasonality.jpg")
```
...
The seasonal naive method allows non-constant predictive values by taking the same values from the time period in the previous season. The drift method is another variation of the naive method that allows the forecasts to increase or decrease over time. To be more specific, we choose the line connecting the first and last observations.  Thus the forecast for time $T + h$ is given by $$\hat{y}_{T+h|T} = y_T +h(y_T-y_1)/(T-1)$$

where the amount of change over time $y_T-y_1$ is called the drift. This is where the name of the method comes from.  


```{r echo = FALSE, fig.align='center', fig.width=5, fig.height=5}
include_graphics("w11-drift.jpg")
```
