...
\right.
$$
The above system can be written in the following matric form

$$
\mathbf{X}^T(\mathbf{Y} - \mathbf{\mu}) = \mathbf{0}
$$
where
* $\mathbf{X}$ is the $n\times (p+1)$ design matrix.
* $\mathbf{Y}$ is the $n\times 1$ vector of the observed responses.
* $\mathbf{\mu}$ is the vector pf means, with $\mu_i = \exp(\mathbf{x}_i^T\beta)$

However, the variance-covariance matrix of the estimator changes because the model-based variance 
$Var(Y_i) = \mu_i$ is replaced by the quasi-variance $Var(Y_i) = \phi\mu_i$ .
...
$$
w_i = \frac{1}{Var{Y_i}\left(\frac{d \mu_i}{d\eta_i}\right)^2} = \frac{1}{Var(Y_i)}\cdot \left(\frac{d \mu_i}{d\eta_i} \right)^2 = \frac{\mu_i}{\phi}
$$

Thus, the weight matrix $\mathbf{W}$ for the Quasi-Poisson model is:

$$
\mathbf{W} = \frac{1}{\phi}\text{diag}(\mu_1, \mu_2, \cdots, \mu_n).
$$
