...
for(i in 1:k){
  ...
  candidate02 = stepAIC(candidate01, 
   ...
   pre.outcome01 = ifelse((pred01) > 0.4, "pos", "neg")
   pre.outcome02 = ifelse((pred02) > 0.4, "pos", "neg")
   pre.outcome03 = ifelse((pred03) > 0.4, "pos", "neg")
   
   PE1[i] = 1-sum(pre.outcome01 == as.vector(valid$diabetes))/length(pred01)
   PE2[i] = 1-sum(pre.outcome02 == as.vector(valid$diabetes))/length(pred02)
   PE3[i] = 1-sum(pre.outcome03 == as.vector(valid$diabetes))/length(pred03)
}
avg.pe = cbind(PE.1 = mean(PE1), PE.2 = mean(PE2), PE.3 = mean(PE3))
pander(avg.pe, caption = "Average of prediction errors of candidate models")
...
pander(accuracy, caption="The actual accuracy of the final model")
```
